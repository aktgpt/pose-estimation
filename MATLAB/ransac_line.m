 function [inlierPts] = ransac_line(data,num,iter,threshDist,inlierRatio)
...
%  figure;plot(data(1,:),data(2,:),'o');hold on;
 ...
 %% Update the number of inliers and fitting model if better model is found     
     if inlierNum>=round(inlierRatio*number) && inlierNum>bestInNum
         bestInNum = inlierNum;
         mLine = (sample(2,2)-sample(2,1))/(sample(1,2)-sample(1,1));
         parameter2 = sample(2,1)-mLine*sample(1,1);
         bestParameter1=mLine; bestParameter2=parameter2;
     end
 end
